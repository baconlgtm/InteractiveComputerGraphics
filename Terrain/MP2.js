...
var eyePt = glMatrix.vec3.fromValues(0,0,0);//(0.0,-0.45,-1.3);
/** @global Direction of the view in world coordinates */
var viewDir = glMatrix.vec3.fromValues(0,0,-1);//(0.0,0.1,-1.5);
...
function setupShaders() {
  ...
  shaderProgram.uniformlowHeight = gl.getUniformLocation(shaderProgram, "ulowHeight");
  shaderProgram.uniformhighHeight = gl.getUniformLocation(shaderProgram, "uhighHeight");
...
function setMaterialUniforms(alpha,a,d,s) {
  ...
  var low, high;
  [low, high] = myTerrain.getMinMaxHeight();
  gl.uniform1f(shaderProgram.uniformlowHeight, low);
  gl.uniform1f(shaderProgram.uniformhighHeight, high);
...
function draw() { 
    ...
    if (currentlyPressedKeys["ArrowLeft"]) {
      rollDegree -= 1;
      glMatrix.quat.setAxisAngle(quaternion, viewDir, degToRad(-1));
    ...
    if (currentlyPressedKeys["ArrowUp"]){
      ...
      glMatrix.quat.setAxisAngle(quaternion, cross, degToRad(1));
    ...
    if(currentlyPressedKeys["ArrowLeft"] || currentlyPressedKeys["ArrowRight"]
      || currentlyPressedKeys["ArrowUp"] || currentlyPressedKeys["ArrowDown"]){
...
//  console.log("Key down ", event.key, " code ", event.code);
  if (event.key == "ArrowLeft" || event.key == "ArrowRight" || event.key == "ArrowUp" || event.key == "ArrowDown" || event.key == "r") {
...
function resetPosition(){
    if (currentlyPressedKeys["r"]){
        ...
        pitchDegree = 0;
        speed = 0.000;
        eyePt = glMatrix.vec3.fromValues(0,0,0);//(0.0,-0.45,-1.3);
        viewDir = glMatrix.vec3.fromValues(0,0,-1);//(0.0,0.1,-1.5);
